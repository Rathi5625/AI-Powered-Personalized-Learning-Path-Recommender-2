...
/**
 * HTTPS Transactional Email Service using Resend REST API (Port 443).
 * Compatible with cloud hosting environments where outbound SMTP ports are blocked.
 */
public class ResendHttpsEmailService implements EmailService {

    private static final Logger log = LoggerFactory.getLogger(ResendHttpsEmailService.class);
    ...
    public ResendHttpsEmailService(String apiKey, String fromAddress, String fromName, ObjectMapper objectMapper) {
    ...
    public ResendHttpsEmailService(String apiKey, String fromAddress, String fromName, ObjectMapper objectMapper, HttpClient httpClient) {
    ...
    public void sendOtp(String toEmail, String code, OtpPurpose purpose) {
        log.info("[EMAIL] Initiating Resend HTTPS delivery for {} OTP to recipient {}", purpose, toEmail);

        if (apiKey.isBlank()) {
            log.error("[EMAIL ERROR] Resend API key is missing. Configure RESEND_API_KEY or EMAIL_PROVIDER_API_KEY in environment variables.");
            throw new ExternalServiceException("Email delivery failed: Resend API key is not configured.", 502);
        }
        ...
        try {
            ...
            if (statusCode >= 200 && statusCode < 300) {
                ...
                log.info("[EMAIL] Resend HTTPS delivery SUCCESSFUL: message accepted by provider for recipient {} (id={})", toEmail, messageId);
            } else if (statusCode == 401) {
                log.error("[EMAIL ERROR] Resend API authentication failed (HTTP 401). Verify RESEND_API_KEY / EMAIL_PROVIDER_API_KEY in Render.");
                throw new ExternalServiceException("Failed to deliver verification email due to email provider authentication failure.", 502);
            } else {
                log.error("[EMAIL ERROR] Resend API delivery failed with HTTP status {}: {}", statusCode, responseBody);
                throw new ExternalServiceException("Failed to deliver verification email via Resend (HTTP " + statusCode + ").", 502);
            }

        } catch (ExternalServiceException ese) {
            throw ese;
        } catch (Exception e) {
            log.error("[EMAIL ERROR] Resend HTTPS request failed for recipient {}: [{}] {}", toEmail, e.getClass().getSimpleName(), e.getMessage());
